"""unitok_drive_lite 的 selective attention mask 实现。

所有 token 类型定义、mask 构造、可视化函数均在本文件内自包含，
不依赖顶层 models/ 目录。
"""
...
# ---------------------------------------------------------------------------
# 对外接口：4D additive mask（训练 / 全序列前向）
# ---------------------------------------------------------------------------
...
def build_selective_attention_mask(
    ...
    - 对 generate(...) 而言，Transformers 通常更稳定地支持 2D padding mask，
      因此不建议直接复用 4D mask
    """
    token_types = _normalize_token_types(token_types)
...
# ---------------------------------------------------------------------------
# 对外接口：2D padding mask（generate 路径）
# ---------------------------------------------------------------------------
